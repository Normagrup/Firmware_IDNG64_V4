...
void normaLinkCfgInfoPasarelaT4(unsigned char _comL)
{
	...
	if ((_comL==0x21) && (_lenght==0x0F))
	{
		...
		{
			rtccWeekdaySet(recibireth[7]);
			rtccDaySet(recibireth[8]);
			rtccMonthSet(recibireth[9]);
			rtccYearSet(recibireth[10]);
			ethSendACK(&recibireth[2],&ipDest[0]);					
		}
	
	if ((_comL==0x33) && (_lenght==0x02))
		{
			rtccHourSet(recibireth[7]);
			rtccMinutesSet(recibireth[8]);
			rtccSecondsSet(recibireth[9]);
			ethSendACK(&recibireth[2],&ipDest[0]);			
		}
...
void normaLinkCfgInfoPasarelaT0(unsigned char _comL)
{
	...
	if ((_comL==0xB0) || (_comL==0xC0)) 	// Iniciar Embedded Bootloader Write
	{
		if (_comL==0xB0)		embeddedBootloaderWriteStart();
		if (_comL==0xC0)		eBootloaderTransceiver_WriteStart();		
	ethSendACK(&recibireth[2],&ipDest[0]);		
	}	
	
	if ((_comL==0xB2) || (_comL==0xC2))	// Iniciar Embedded Bootloader Read
	{
		if (_comL==0xB2)	embeddedBootloaderReadStart();
		if (_comL==0xC2)	eBootloaderTransceiver_ReadStart();
	ethSendACK(&recibireth[2],&ipDest[0]);			
	}	
	
	if ((_comL==0xB3) || (_comL==0xC3))	// Leer
	{
		unsigned char readBuffer[256];
		unsigned int n;		
		if (_comL==0xB3)	embeddedBootloaderRead(&readBuffer[0]);
		if (_comL==0xC3)	eBootloaderTransceiver_Read(&readBuffer[0]);		
		_tipoDeFrame=0x83;		
		_respuesta[0]=_tipoDeFrame;
		_respuesta[1]=recibireth[2];
		_respuesta[2]=recibireth[3];	
