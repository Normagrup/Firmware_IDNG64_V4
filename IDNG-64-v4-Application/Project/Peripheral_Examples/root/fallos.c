...
// 64 + 64 + 64 + 64 + 64 + 64 + 1			// 385 bytes 				49¿?
...
void fallosDoTask (void)
{
	...
	{
		historicoAdd (0xFF, 0xFF, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), HISTORICO_INICIO_CENTRAL);			
	...
	if (deviceType==0x01)		// E m e r g e n c i a s
	{
		...
		else
		{
			if (fallosAutonomiaEstado[address]!=FALLOS_ESTADO_NO_MARCADO)
			{
				...
				{
					if (historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), HISTORICO_RECUPERACION_FALLO_AUTONOMIA)==0x01)
					{
		...
		else
		{
			if (fallosBateriaEstado[address]!=FALLOS_ESTADO_NO_MARCADO)
			{
				...
				{
					if (historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), HISTORICO_RECUPERACION_FALLO_BATERIA)==0x01)
					{
		...
		else
		{
			if (fallosLamparaEstado[address]!=FALLOS_ESTADO_NO_MARCADO)
			{
				...
				{
					if (historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), HISTORICO_RECUPERACION_FALLO_LAMPARA)==0x01)
					{
...
{	
	...
	else
	{
		if (fallosComunicacionEstado[address]!=FALLOS_ESTADO_NO_MARCADO)
		{
			...
			{
				if (historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), HISTORICO_RECUPERACION_FALLO_COMUNICACION)==0x01)
				{				
