...
{	
	...
	{
		historicoAdd (0x00, ((dest&0x7E)>>1), rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), event);						
	}
	if (destTipo==DEST_TIPO_GRUPO)	
	{
		...
		{				
			...
			{
				historicoAdd (0x00, n, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), event);					
			}
		}
	}
	...
	{	
		for (n=0;n<64;n++)
		{					
			 if ((daliLoopInstalledGet(n)==0x01) && (daliLoopDeviceTypeGet(n)==0x01))		historicoAdd (0x00, n, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(), event);								
		}
	}	
}
...
{		
	...
	else																										
	{
		...
		{		
			historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(),  HISTORICO_REALIZADO_FT);						
	...
	else																										
	{
		...
		{		
			historicoAdd (0x00, address, rtccDayGet(), rtccMonthGet(), rtccYearGet(), rtccHourGet(), rtccMinutesGet(),  HISTORICO_REALIZADO_DT);							
